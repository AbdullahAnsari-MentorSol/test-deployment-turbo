...
export function PersonalAccountCheckoutForm(props: {
  ...
  subscription?: Subscription & { items: LineItem[];} | null | undefined;
}) {
  ...
  // There is only one subscription, so we extract the first (and only) plan id
  const currentPlanId = props.subscription?.items[0]?.variant_id || null;

  const filteredProducts: typeof billingConfig.products = billingConfig.products.filter((product) => {
    const hasSubscribedPlan = product.plans.some((plan) => plan.id === currentPlanId);
    ...
    <div>
      <Card>
        ...
        <CardContent className={'space-y-4'}>
          ...
          <PlanPicker
            pending={pending}
            config={{ ...billingConfig, products: filteredProducts }}
            ...
            onSubmit={({ planId, productId }) => {
              startTransition(async () => {
                try {
                  ...
                  const { checkoutToken } = await createPersonalAccountCheckoutSession({planId,productId,});
