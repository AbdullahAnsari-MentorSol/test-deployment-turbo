 ...
 * @description Creates a customer on Stripe and returns the customer details to store in the database.
 * @param {object} params - The parameters to create the customer, such as email and name.
 * @returns {object} The customer object containing customer details like ID and email.
 */
async createStripeCustomer(params: z.infer<typeof CreateCustomerParamsSchema>) {
  const stripe = await this.stripeProvider(); 
  const logger = await getLogger();  
  ...
  logger.info(ctx, 'Creating customer on Stripe...');
  ...
  try {
    ...
    logger.info(ctx, 'Customer created successfully', { customerId: customer.id });
    ...
    logger.error(ctx, 'Failed to create customer on Stripe', { error });
  ...
  async getSubscription(subscriptionId: string) {
    ...
    try {
      ...
      logger.error({ ...ctx, error }, 'Failed to retrieve subscription dsdsdsdsd');
