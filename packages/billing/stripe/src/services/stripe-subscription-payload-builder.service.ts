...
import { BillingConfig, getCreditsById, getLineItemTypeById, getTierById } from '@kit/billing';
...
class StripeSubscriptionPayloadBuilderService {
  ...
  >(params: {
    ...
    const tier = this.config && params.lineItems[0] ? getTierById(this.config, params.lineItems[0].price?.id as string): "Free";
    const lineItems = params.lineItems.map((item) => {
      ...
      return {
        ...
        price_amount: item.price?.unit_amount && item.price?.unit_amount/100,
