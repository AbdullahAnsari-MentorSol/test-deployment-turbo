...
export class AdminDashboardService {
  ...
  async getDashboardData(
    ...
    const subscriptionsStarterPromise = this.client
      .from('subscriptions')
      .select('*', selectParams)
      .eq('status', 'active')
      .eq('tier', 'Starter')
    ...
    const subscriptionsProPromise = this.client
      .from('subscriptions')
      .select('*', selectParams)
      .eq('status', 'active')
      .eq('tier', 'Pro')
    ...
    const subscriptionsEnetrprisePromise = this.client
      .from('subscriptions')
      .select('*', selectParams)
      .eq('status', 'active')
      .eq('tier', 'Enterprise')
    ...
    const [subscriptions, trials, accounts, teamAccounts,starterSubscriptions,proSubscriptions,enterpriseSubscriptions] = await Promise.all([
      subscriptionsPromise,
      trialsPromise,
      accountsPromise,
      teamAccountsPromise,
      subscriptionsStarterPromise,
      subscriptionsProPromise,
      subscriptionsEnetrprisePromise,
